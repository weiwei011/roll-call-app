...
def load_data():
    ...
    try:
        ...
        if "ManualStart" not in df.columns: df["ManualStart"] = ""
        if "ManualEnd" not in df.columns: df["ManualEnd"] = ""
        if "ManualReason" not in df.columns: df["ManualReason"] = ""
        
        return df.fillna("")
    except Exception as e:
        st.error(f"讀取失敗，載入暫存模式: {e}")
        df = pd.DataFrame(DEFAULT_ROSTER)
        df["Schedule"] = "[]"
        df["ManualStart"] = ""
        df["ManualEnd"] = ""
...
# ==========================================
# 邏輯 A: 全能相容模式 (Line 機器人 & APP 整合)
# ==========================================
def get_person_status(row):
    """
    判定人員狀態
    優先序：
    1. Line/手動輸入欄位 (ManualStart/End)
    2. APP JSON 排程 (Schedule)
    3. 未來行程預告
    """
    now = get_taiwan_time()
    
    # --- 1. 檢查 Line/Manual 欄位 (E, F, G 欄) ---
    ...
    if pd.notna(m_start) and str(m_start).strip() != "" and \
       pd.notna(m_end) and str(m_end).strip() != "":
        try:
            ...
            if ms_dt <= now <= me_dt:
                final_reason = str(m_reason) if (pd.notna(m_reason) and str(m_reason).strip() != "") else "Line假單"
                s_str = ms_dt.strftime('%d日 %H%M')
                return "leave", final_reason, f"🔴 {final_reason} ({s_str}~)", None
        except:
            pass # 格式錯誤則忽略

    # --- 2. 檢查 APP Schedule 欄位 (D 欄) ---
...
def parse_multi_incident_input(text_input, current_df):
    ...
    for line in lines:
        ...
        # 2. 抓取時間 (例如: 11/28 1600 - 12/03 0730)
        ...
        if match:
            ...
            # 跨年邏輯
            y1, y2 = current_year, current_year
            ...
            try:
                t1_str = f"{t1.zfill(4)[:2]}:{t1.zfill(4)[2:]}"
                t2_str = f"{t2.zfill(4)[:2]}:{t2.zfill(4)[2:]}"
...
def apply_batch_leave_manual(cats, start_dt, end_dt, reason, current_df):
    ...
    for i, row in current_df.iterrows():
        ...
        conflict = False
        for e in schedule:
             if max(start_dt, datetime.datetime.fromisoformat(e['start'])) < min(end_dt, datetime.datetime.fromisoformat(e['end'])):
                 conflict = True
                ...
                with cols[i % 3]:
                    ...
                    with st.popover(f"管理 {row['Name']}"):
                        ...
                        if pd.notna(m_start) and str(m_start).strip() != "":
                            st.info("📱 **Line / 手動輸入資料**")
                            st.write(f"**事由**: {row.get('ManualReason')}")
                            st.write(f"**時間**: {m_start} ~ {row.get('ManualEnd')}")
...
with tab2:
    ...
    if st.button("🚀 批次更新", type="primary"):
        if raw_df.empty: st.error("無資料")
            ...
            if count > 0: 
                save_data(new_df)
                st.success(f"已新增 {count} 筆")
                time.sleep(1)
                st.rerun()
            else: st.warning("無更新或格式無法辨識")
...
with tab3:
    ...
    with st.container(border=True):
        ...
        if st.button("⚡ 執行放假 (依標籤)", type="primary", use_container_width=True):
            if not selected_groups: st.error("請選擇群組")
            else:
                new_df, count = apply_routine_leave(selected_groups, raw_df.copy())
                if count > 0: 
                    save_data(new_df)
                    st.success(f"已設定 {count} 員")
                    time.sleep(1)
                    st.rerun()
                else: st.warning("無變更")

    st.divider()
    with st.expander("🛠️ 自訂時間放假 (手動指定)"):
        with st.form("custom"):
            c1, c2 = st.columns(2)
            ...
            if st.form_submit_button("執行"):
                ...
                if dte<=dts: st.error("結束時間不能早於開始時間")
                else:
...
with st.sidebar:
        ...
        with st.form("add"):
            ...
            if st.form_submit_button("新增"):
                if nn and nn not in raw_df['Name'].values:
                    new_row = {"Category": nc, "Name": nn, "Tag": nt, "Schedule": "[]", "ManualStart": "", "ManualEnd": "", "ManualReason": ""}
                    save_data(pd.concat([raw_df, pd.DataFrame([new_row])], ignore_index=True))
                    st.rerun()
                else:
                    st.error("姓名重複或為空")
    ...
    with st.expander("🔴 危險操作 (重置)"):
        ...
        st.caption("若名單順序亂掉，點此恢復預設")
        if st.button("🗑️ 刪除全部並載入預設名單", type="primary"):
            # 清空資料庫，觸發下次 load_data 重建
            save_data(pd.DataFrame(columns=["Category", "Name", "Tag", "Schedule", "ManualStart", "ManualEnd", "ManualReason"]))
            time.sleep(1)
            st.rerun()
